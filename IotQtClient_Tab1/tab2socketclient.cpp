    ...
    ui(new Ui::Tab2SocketClient)
{
    ...
    connect(ui->pBServerConnect,SIGNAL(clicked(bool)),this,SLOT(slotConnectToServer(bool)));
...
void Tab2SocketClient::slotConnectToServer(bool check)
{
    ...
    if(check){
        ...
        if(bOk){
            ui->pBServerConnect->setText("서버 해제");
            ui->pPBsendButton->setEnabled(true);
        } else{
            ui->pBServerConnect->setChecked(false);
        }
    } else{
        ui->pBServerConnect->setText("서버 연결");
